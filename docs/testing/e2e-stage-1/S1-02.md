...
**FALTA**: tienda inactiva → 403, y el rate limit de login (inalcanzable bajo `Testing`, ver **H-12**).

**E2E frontend (Playwright) — PENDIENTE**
Ninguna de las aserciones, incluidas las 6 de destino post-login. Existe cobertura `vitest` en `auth-store.test.ts:125` y `login.test.tsx`, pero ninguna de navegador ni de destino.
