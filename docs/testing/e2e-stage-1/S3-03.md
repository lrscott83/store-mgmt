...
**MEDIDO desde entonces** (`Users/UsersIsolationTests.cs`), y el resultado sale partido:

- **Cross-tenant: SÍ aísla.** `Update_owner_admin_updates_user_in_other_tenant_returns_envelope_404` — HTTP 200 con `actionCode: 404` en el envelope, y el usuario ajeno **no se modifica** en base.
- **Cross-store: NO aísla.** `Update_owner_admin_updates_user_in_other_store_returns_200` — la edición se aplica. El límite de tienda no se hace cumplir.

O sea que **H-11 queda partida**: la mitad de tenant está cerrada; la de tienda es un defecto abierto de producción, no un hueco de cobertura.
