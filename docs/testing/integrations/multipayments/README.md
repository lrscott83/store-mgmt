...
Afirmación: *con el módulo de multipagos activo, pagar una venta en dos partes por canales distintos
recalcula bien el total y el vuelto, y no deja registrar una venta pagada de menos.*
...
Entrada movida desde `docs/testing/known-issues/group-anotados/multipayments-t10-2.md`:

- **Qué falla (corrida 2026-09-24):** antes de llegar al carrito, el test tiene que registrar una tasa
  de cambio en la pantalla de "tasas de cambio"; el formulario nunca aparece (el campo donde se
  escribe la tasa no se dibuja) y el test muere ahí.
- **Causa raíz:** ⏸ **anotada, no confirmada** — (1) el spec se reescribió en dev el 23/09 ("currency
  block and popup flow") y la versión que falló ya no existe en el repo; (2) la pantalla de tasas
  también lee datos guardados, y si esa lectura se hiciera al pintar sería el mismo bug del Grupo A
  (ya arreglado en `StorePaymentMethodsConfigService`).
- **Propuesta:** correr una vez el spec reescrito; si el formulario sigue sin aparecer, aplicar el
  mismo arreglo del Grupo A y re-verificar.
- **Estado:** ⏸ pendiente de decisión (requiere autorización de corrida).
