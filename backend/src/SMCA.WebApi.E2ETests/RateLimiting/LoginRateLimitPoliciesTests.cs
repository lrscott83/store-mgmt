...
public class LoginRateLimitPoliciesTests
{
    ...
    [Fact]
    public void Login_policy_options_match_production_config()
    {
    ...
    [Fact]
    public void Login_policy_limiter_behavior_matches_options()
    {
        var limiter = BuildLimiter(RateLimitPolicies.Login(ContextWithIp(null)));

        // Window / SegmentsPerWindow = 1min / 3 = 20s replenishment period
        ...
        var act = () => limiter.AttemptAcquire(6);
        act.Should().Throw<ArgumentOutOfRangeException>()
            .WithMessage("*permit limit of 5*");

        // Exactly PermitLimit permits can be acquired at once
        using (var full = limiter.AttemptAcquire(5))
        {
    ...
    [Fact]
    public void Login_policy_partition_key_is_per_ip()
    {
        var partitionA = RateLimitPolicies.Login(ContextWithIp("203.0.113.10"));
        var partitionB = RateLimitPolicies.Login(ContextWithIp("203.0.113.11"));
    ...
    [Fact]
    public void Login_policy_null_ip_maps_to_unknown_partition()
    {
        var partition = RateLimitPolicies.Login(ContextWithIp(null));
