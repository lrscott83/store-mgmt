...
- Verify verdict: BLOCKED overridden by the orchestrator. All 5 requirements
  in this capability verified true against code (16/16 call sites, DEK-first
  ordering, marker-first decrypt, all 6 auto-init writes confirmed outside
  their read's `try/catch`). The blocking CRITICAL belongs to a sibling
  capability's KAT-fixture provenance (see `dek-lifecycle-and-unlock-gate`),
  not to this capability. See that change's archive report for the full
  override rationale.
