# CSV Product Import — Parsing Specification
...
- MUST NOT add a `papaparse` dependency — hand-rolled RFC4180 tokenizer only (verified: zero
  `papaparse` entries in any `frontend-react` package.json/lockfile).

## Known Deviation (pre-existing, non-blocking, ratified)

The per-row error-diagnostics system (`MISSING_NAME`/`MISSING_PRICE`/`INVALID_PRICE`/
`MISSING_CATEGORY`) is itself a React-only enhancement relative to Angular's `validateProducts`,
which silently filters invalid rows with zero per-row diagnostics (a single `&&` boolean filter).
This predates the `MISSING_CATEGORY` addition and is not a regression — flagged only for a future
dedicated audit (verify report SUGGESTION-2, 2026-07-15).
