...
- [x] E2E: `GET /api/v1/stores/{id}` returns 200 with `ownerName` populated (no NRE) — R1
- [x] Unit: handler class name is `GetStoreByIdQueryHandler` — R2
- [x] Unit: handler return has no `await Task.FromResult` — R3
- [x] Unit: validator executes `AnyAsync`, not full Include query — R4
- [x] Integration: `ExistsAsync` returns true/false correctly — R4
- [x] Swagger: 401/403/400 response codes documented — R5
- [x] Code review: XML summary present — R6
- [x] Code review: namespace is `Application.Services.Stores` — R7
- [x] E2E: deleted store (race) returns 404, not NRE — R8
