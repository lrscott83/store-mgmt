...
This capability's error idiom differs from `admin-owners-resellers`/`admin-stores`: those use try/catch, `user-list.tsx` uses `.then/.catch`. The `succeeded: false` check belongs inside the existing `.then` callback, not a new catch branch — no new async pattern is introduced. `handleLifecycleAction`'s existing try/catch (`USERS.LIFECYCLE_ERROR`) is unaffected — out of scope for this requirement.
