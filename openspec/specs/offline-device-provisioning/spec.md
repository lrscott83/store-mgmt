...
- **Open at archive — NOT closed by this change.** The "Admin export action"
  requirement is **BLOCKED-for-verification**:
  `GET /v1/storeusers/{storeId}/offline-roster` does not exist server-side.
  Unit tests prove only URL construction and `response.data.data`
  unwrapping against a mock. Response envelope, DTO field casing,
  epoch-ms-vs-ISO dates, and whether `users[].verifier` exists at all are
  UNPROVEN. This is a BACKEND item, tracked in
  `docs/plans/2026-07-28-backend-pending-work.md` §7a, and is out of scope
  for the frontend change that produced this spec. The
  `BLOCKED-for-verification` comments in `roster-http-service.ts` and its
  test document this explicitly and MUST NOT be softened or removed until
  the backend endpoint ships and end-to-end verification actually runs.
- Manual smoke of the full provisioning flow (export → transfer → import →
  offline login) is also unexecuted — see `auth-session`'s Verification
  Status for the consolidated manual-smoke tracking note.
