...
**Origin**: SDD change `users-e2e` (extended by `e2e-stage-1-s3-03`, 2026-08-06)  
**Status**: Active  
...
| Cross-tenant update (E2E-I1) | OwnerAdmin+ProfileAdmin | 200, envelope ActionCode 404 + no DB write (tenant-isolation invariant guard) |
| Cross-store same-tenant update (E2E-I2) | OwnerAdmin+ProfileAdmin | 200, DB write persists (isolation is tenant-only, NOT store-level) |
...
## Delta for users-e2e: E2E Isolation on PUT /v1/users/{id} — Cross-Tenant Invariant + Cross-Store Coverage

**Change**: `e2e-stage-1-s3-03`

Extends the users-e2e enforcement window with 2 ADDED requirements that pin the tenant-isolation invariant on `PUT /v1/users/{id}` (H-11). No product behavior is added or modified — the requirements describe E2E coverage of existing behavior.
