...
- [x] `signedInPage` existe en `support/test.ts` y cumple REQ-1..REQ-6 — verificado por lectura completa del diff + corrida en vivo del bloque `describe.serial` de `login.spec.ts` que la consume (2026-08-07)
- [x] La costura declarada en `support/test.ts:10-13` queda resuelta (comentario original reemplazado o confirmado, no solo dejado como TODO) — verificado
- [x] `registerNetwork` conserva `auto: true` y su comportamiento exacto (no roto por esta fixture) — verificado, diff de `test.ts` estrictamente aditivo, bloque `registerNetwork` byte-idéntico a `main`
- [x] `register.spec.ts` sigue verde tras la adición — verificado en vivo, 8/8
