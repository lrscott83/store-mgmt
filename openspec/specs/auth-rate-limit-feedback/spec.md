...
- **login.tsx 429 branch**: Done (`app/auth/routes/login.tsx`)
- **register.tsx 429 branch**: Done (`app/auth/routes/register.tsx`)
- **i18n keys**: Done — `AUTH.TOO_MANY_ATTEMPTS` (voseo, `shared/lib/i18n/es.ts`),
  `REGISTRATION.TOO_MANY_ATTEMPTS` (usted/tuteo with "Por favor," prefix, matching the
  `REGISTRATION.*` namespace's existing voice — NOT the `AUTH.*` namespace's voseo)
- **Tests**: Done (`login.test.tsx`, `register.test.tsx`) — 429 cases plus regression coverage for
  401/403/400 branches

## Notes

- Added by SDD change `register-endpoint-contract-frontend` (archived 2026-07-31).
- No rate-limit figures (e.g. "5 per minute") are quoted in the copy — the frontend does not read
  `Retry-After` and the backend's exact window (`Program.cs`) is not surfaced to the user; copy
  uses "un momento" / "unos minutos" instead.
- `AUTH.*` and `REGISTRATION.*` are deliberately different voices (voseo vs. usted/tuteo), matching
  each namespace's pre-existing style — not a single house style across both pages.
- `api-client.ts` is untouched by this capability; each page's own catch handler owns its 429
  branch.
