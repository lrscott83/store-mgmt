...
> **Nota de estado (archive, 2026-08-07)**: implementado, tipa correctamente y aislado por tag/script tal como exige este requirement. **No verificado en ejecución** — ver `archive-report.md` de este cambio para el detalle de la brecha aceptada.
...
- [x] REQ-1..REQ-7, REQ-9..REQ-14 corren en la corrida por defecto (`pnpm test:e2e`) — verificado en vivo, 2026-08-07 (suite por defecto 20/20 verde contra backend real, `login.spec.ts` incluido con sus 8 tests)
- [x] REQ-8 corre solo mediante su propio script (`pnpm test:e2e:rate-limit`), con umbrales 5/1min — nunca los de `RegisterPolicy` — verificado estáticamente (tag `@rate-limit`, excluido de `test:e2e`, `MAX_ATTEMPTS = 7`); **NO verificado en ejecución** (ver archive-report)
- [x] REQ-15 verificado: ningún test escribe `localStorage` de productos/categorías a mano — confirmado por lectura de código (`store-seed.ts` 100% UI-driven) y por la corrida en vivo (D1/D3 aterrizan en `/sales/new` como se espera)
- [ ] REQ-16 verificado manualmente por el usuario forzando un fallo de siembra — pendiente
- [x] `register.spec.ts`, `register-rate-limit.spec.ts`, `smoke.spec.ts`, `api-health.spec.ts` permanecen sin cambios — verificado, `git diff` vacío para los cuatro + `--list` sin cambios (8/8)
- [x] Ningún fichero `vitest` de login fue tocado — verificado, `git diff --stat -- '**/*.test.ts' '**/*.test.tsx'` vacío
