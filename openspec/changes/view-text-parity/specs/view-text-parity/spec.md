...
| Non-404 / generic details | `Por favor, vuelva a intentarlo y si persiste el error contacte al equipo de soporte técnico.` | `GENERAL.RESPONSE.ERROR500_MESSAGE` (256) |
...
| Element | Current (English) | Required (Spanish) |
|---|---|---|
...
- `login.tsx` input `type`/`autoComplete` (stays `email`), and `MENU.TUTORIAL`
  aria-label in navbar.tsx (already Spanish via existing key — not a gap).
- Dynamic/backend-sourced error banner text (API response messages) — out of scope;
  only static fallback/UI copy is specified here.
