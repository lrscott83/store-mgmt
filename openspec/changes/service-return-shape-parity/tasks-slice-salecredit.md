...
- [x] 1.1 RED: `packages/domain/src/errors/__tests__/sale-credit-errors.test.ts` asserts
...
- [x] 1.2 GREEN: create `packages/domain/src/errors/sale-credit-errors.ts` (same `as const
...
- [x] 1.4 Gate: `pnpm test`, `tsc --noEmit`; commit `feat(domain): port SaleCreditErrors.NotExists
...
- [x] 2.1 RED/GREEN: rename `createFromOrder(orderId, client, total)` →
...
- [x] 2.2 RED/GREEN: rename `update(id, client, note)` → `updateSaleCredit(id, client, note):
      ...
      `throw new Error`).
- [x] 2.3 RED/GREEN: rename `pay(id, paidType, note)` → `paidSaleCredit(id, paidType, note):
      DataResult<SaleCredit>` — same success/failure shape as 2.2, never throws.
- [x] 2.4 Call-site: `today-credits.tsx` `handleSave`/`handlePay` — replace try/catch with `const
      result = svc.updateSaleCredit(...)` / `svc.paidSaleCredit(...)`; `return result.succeeded`
      (flagged #5).
- [x] 2.5 RED/GREEN: add `deleteSaleCredit(id): Result` — missing id →
...
- [x] 2.6 RED/GREEN: rename `voidByOrderId(orderId)` → `deactivateSaleCreditByOrderId(orderId):
...
- [x] 2.7 RED/GREEN: adapt `delete(id): void` (BaseService seam) to call `deleteSaleCredit(id)` and
...
- [x] 2.8 RED/GREEN: add `addImportedSaleCredit(saleCredit): Result` and
...
- [x] 2.9 Gate: `pnpm test`, `tsc --noEmit`, build; commit `feat(domain,web-store-pos): restore
      SaleCredit category-D Result/DataResult shapes`.

## WU3: Category B — Sync `BaseResponseModel<T>` Envelope — Req: Category B SYNC

- [x] 3.1 RED/GREEN: add `getSaleCreditsInDay(date): BaseResponseModel<SaleCredit[]>` via
...
- [x] 3.2 GREEN: remove `getByDateRange`/`getActiveToday` (no Angular correlate, flagged #2).
- [x] 3.3 Gate: `pnpm test`, `tsc --noEmit`, build; commit `feat(domain,web-store-pos): restore
...
- [x] 4.1 RED/GREEN: add `getSaleCreditsInDayObservable(date):
      Promise<BaseResponseModel<SaleCredit[]>>` — `Promise.resolve(this.getSaleCreditsInDay(date))`,
      EXACT Angular name.
- [x] 4.2 RED/GREEN: convert `getUnpaidCreatedToday()` → `getUnPaidSaleCreditsInDayObservable(date):
...
- [x] 4.3 RED/GREEN: convert `getPaidToday()` → `getPaidSaleCreditsInDayObservable(date):
...
- [x] 4.4 RED/GREEN: add `getSaleCreditsObservable(): Promise<BaseResponseModel<SaleCredit[]>>` —
...
- [x] 4.5 RED/GREEN: convert `filterSaleCredits(isPaid, client?, start?, end?)` (currently sync) →
...
- [x] 4.6 Call-site: `today-credits.tsx` `loadSaleCredits` — `async`, `await
      svc.getSaleCreditsInDayObservable(new Date())`, `.data` unwrap on `.succeeded` (flagged #2).
- [x] 4.7 Call-site: `credits.tsx` `loadSaleCredits` — `async`, replace `getAll().filter(isActive)`
      with `await svc.filterSaleCredits(undefined, undefined, undefined, undefined)`, `.data`
      unwrap (flagged #4).
- [x] 4.8 Call-site: `today-stats.tsx` credits branch inside the `useEffect` — inner async IIFE,
...
- [x] 4.9 Gate: `pnpm test`, `tsc --noEmit`, build; commit `feat(web-store-pos): restore SaleCredit
      category-C async envelope + call-sites`.

## Test Rework Closeout (folded into WU2-4 commits)

- [x] 5.1 `sale-credit-offline-service.test.ts`: rework `createFromOrder`/`update`/`pay`/`delete`
      ...
      `getPaidToday` blocks.
- [x] 5.2 `order-offline-service.test.ts`: rework the `SaleCreditOfflineService` mock (rename
...
- [x] 5.3 `sales-routes.test.tsx`: rework the `SaleCreditOfflineService` mock block (remove
...
- [x] 5.4 `today-stats.test.tsx`: replace `getUnpaidCreatedToday`/`getPaidToday` mocks with
      ...
      expected totals.
- [x] 5.5 Verify `sale-credit-list.tsx`/component tests need no rework (presentational, receives
...
- [x] 6.3 Update this file with commit hashes; confirm flagged mismatches #1/#6/#8 resolutions
      taken (text preserved, single-match deactivate, sync-rewire deferred).

## Status: COMPLETE (Slice 2b — SaleCredit).

### Commit log (feat/frontend-parity-audit, commits-only)

| WU | Commit | Subject |
|----|--------|---------|
| WU1 | `3bc845a` | `feat(domain): port SaleCreditErrors.NotExists (byte-identical Angular parity)` |
| WU2 | `c960948` | `feat(domain,web-store-pos): restore SaleCredit category-D Result/DataResult shapes` |
| WU3+WU4 | `8507a2d` | `feat(web-store-pos): restore SaleCredit category-B/C envelope shapes + async call-sites` |

**WU3 and WU4 combined into one commit (deviation from the per-WU plan, justified):** WU3
removes `getByDateRange`/`getActiveToday`. `getActiveToday` is used INTERNALLY by the
pre-WU4 `getUnpaidCreatedToday` AND externally by `today-credits.tsx`'s `loadSaleCredits` —
both are only rewired to the async siblings by WU4. A WU3-only commit would therefore leave
the build broken (unresolved references) — violating the "each commit builds clean" gate.
Landing them together is the only ordering that keeps every commit green (same precedent as
the Expense slice's WU3+WU4 commit).

### Flagged-mismatch resolutions taken

- **#1 (`SaleCreditErrors.NotExists` copy-paste text bug):** PRESERVED byte-identical —
  `"El gasto no existe."`, confirmed against `sale-credit.errors.ts:6`. No scope creep to
  "El crédito no existe".
- **#2 (`getByDateRange`/`getActiveToday` no correlate):** REMOVED; `today-credits.tsx`
  rewired to `getSaleCreditsInDayObservable(new Date())`, matching Angular's real call
  pattern at `today-sale-credits.component.ts:27`.
- **#3 (`getUnpaidCreatedToday`/`getPaidToday` invented sync renames):** converted IN
  PLACE to `getUnPaidSaleCreditsInDayObservable`/`getPaidSaleCreditsInDayObservable`
  (async, Angular-exact names, verified `today-stats.component.ts:92,102`); `today-stats.tsx`
  credits branch now awaits both via `.then()`.
- **#4 (`credits.tsx` bypassing the service filter):** REWIRED — `loadSaleCredits` now
  calls `await service.filterSaleCredits(undefined, undefined, undefined, undefined)`,
  dropping the `getAll().filter(isActive)` bypass (matches Angular's
  `SaleCreditsComponent.loadSaleCredits()` at `sale-credits.component.ts:51-52`).
- **#5 (`createFromOrder`/`update`/`pay` invented names):** renamed to Angular-exact
  `createSaleCredit`/`updateSaleCredit`/`paidSaleCredit`, all returning `DataResult`, never
  throwing. `order-offline-service.ts`'s `create()` calls
  `createSaleCredit(orderId, clientName, total, '')` (Angular always passes `''` for note,
  confirmed `order-offline.service.ts:63`), ignoring the returned `DataResult`.
  `today-credits.tsx`'s `handleSave`/`handlePay` swapped try/catch for a `.succeeded` check.
- **#6 (`voidByOrderId` all-matches-loop bug vs. Angular single-match):** CONFIRMED against
  Angular source (`sale-credit-offline.service.ts:111-118`): `getSaleCreditByOrderId` uses
  `.find()`, not a loop. FIXED (TDD, angular-bugs-policy) — renamed to
  `deactivateSaleCreditByOrderId`, now finds only the FIRST active credit for the orderId
  and soft-deletes just that one via `deleteSaleCredit`; always resolves `Result.Success()`
  even when no credit is found. `order-offline-service.ts`'s `deactivate()` calls
  `deactivateSaleCreditByOrderId(id)`, ignoring the result — Order's own result-checking
  cascade (Angular's `deactivateOrder`, `order-offline.service.ts:317-324`) is explicitly
  DEFERRED to the Order slice per design ADR-4 dependency order, as planned.
- **#7 (`void`/`delete` seam split):** added `deleteSaleCredit(id): Result` (real Angular
  command, never throws, `Result.Failure([SaleCreditErrors.NotExists])` on missing id); the
  internal `void(id)` helper was removed/absorbed — `deactivateSaleCreditByOrderId` now
  calls `deleteSaleCredit` directly (mirrors Angular's own reuse at
  `sale-credit-offline.service.ts:117`). `BaseService` seam `delete(id): void` adapts to
  call `deleteSaleCredit(id)` and throws `result.errors[0]?.description` on failure —
  behavior change (silent no-op → throw) taken, matching the Expense-slice precedent; the
  old "no-op for missing id" test was reworked to assert the new throw (SC-13).
- **#8 (sync import routing, deferred):** `addImportedSaleCredit`/`updateImportedSaleCredit`
  PORTED (1:1, partial-merge semantics for `updateImportedSaleCredit`: only overwrites
  `paid/isPaid/paidDate` when `!existing.paid`) but NOT wired into
  `DataSynchronizerService`/`sync/routes/import.tsx` in this slice — confirmed still
  flagged as a follow-up only, not executed here, per the binding rules.

### Final gate observed

- `packages/domain`: `npx vitest run` → 10 files, 93 tests passed. `tsc --noEmit` clean.
  `pnpm build` (`tsc`) clean.
- `apps/web-store-pos`: `npx vitest run` → 108 files, 1447 tests passed. `npx tsc` (via
  `react-router typegen && tsc`) clean, zero errors. `npx react-router build` clean
  (client + service-worker + SSR-disabled SPA build all succeeded).
- Grep-confirmed (6.1): zero remaining `throw new Error` in
  `sale-credit-offline-service.ts` except the intentional `delete()` seam-throw; zero
  remaining `getByDateRange`/`getActiveToday()`/`getUnpaidCreatedToday`/`getPaidToday`
  references in `app/sales` outside comments and Order's own unrelated `getByDateRange`.
