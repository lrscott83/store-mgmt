...
- [x] 8.2 **AUTOMATED — no longer a manual step.** Cache Storage composition is now a build gate: `verify-sw-precache.mjs` asserts every family in `REQUIRED_PRECACHE_FAMILIES` (`precache-patterns.mjs`) is present at its declared count, failing the build in either direction. Verified failing (`images/**/*.png: expected 7, found 6`) and restored. Covered by `scripts/__tests__/precache-families.test.mjs`.
  - The single cache name is likewise verifiable from the artifact: the built `service-worker.js` contains exactly one `caches.open` and one cache-name literal, `app-shell-v3`, so `app-chunks-v1`/`fonts-v1` cannot be created.
  - **The old wording of this task was wrong** and would have blocked a good ship. It listed ~14 files after "containing", which reads as exhaustive, but the design precaches **141** entries: the enumerated shell assets PLUS 116 route chunks, 8 install icons and 2 CSS files — all of them declared in `PRECACHE_GLOB_PATTERNS`. A human opening Cache Storage would have found 141 where the checklist implied 14. Route chunks and CSS are deliberately left uncounted by the gate because their number moves with every code split.
