# Resources Localization Specification (NEW DOMAIN)
...
Localization resources consumed via `IStringLocalizer` across backend handlers and validators. First spec for this domain — created by this change.
...
- [ ] Both resx contain `CannotDeleteSelf` with exact values; insertion points per RS-1
- [ ] `UserNotFound` renamed in both resx; values + position unchanged
- [ ] Grep `UserNotFoud` → 0 matches in source (Designer.cs stale property expected/optional)
