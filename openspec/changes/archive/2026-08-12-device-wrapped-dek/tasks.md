...
### WU8.1 — Slice B gap closure (follow-up batch, coverage/spec review only)
Files: MODIFIED (append-only) `offline/__tests__/dek-provisioning.test.ts`, `stores/__tests__/auth-store.dek.test.ts`, `stores/__tests__/auth-store.offline.test.ts`. Zero production files touched — all three gaps below were judged CORRECT against spec/design and pinned; none required a production fix. Depends on WU5-WU8 (Slice B, done).
- [x] 8.1.1 GAP 1 (the uncovered crypto branch) — RED-then-GREEN via temporary mutation (production code pre-existed from WU5/WU7; confirmed the new test fails for the right reason by temporarily inverting `dek-provisioning.ts`'s "structural note 1" branch, then reverting): new test in `auth-store.offline.test.ts` (`loginOffline()` on a v2 roster WITH a wrap for this login, no local table yet) exercises the one branch WU7's 7.3 test could not reach (7.3 uses a v1 roster, which never sets a DEK inside `authenticateOffline`). Judged CORRECT against device-dek-wrap spec Requirement "The DEK source is decided once per device" — pinned, no production change.
- [x] 8.1.2 GAP 2 (`sessionStoreId` binding) — verified `user.selectedStoreId` against the repo's established "current store" convention (`user-home.ts:24`, `authorization-service.ts:35`, `use-store-usage-tracker.ts`, `sync/routes/{export,import}.tsx`, `expenses` routes, `statistics/dashboard.tsx`, `sidebar.tsx`) — consistent and CORRECT. Pinned with two new tests (one per call site, RED-confirmed via temporary mutation then reverted): `auth-store.dek.test.ts` 11.5 (online mint branch) and a new `auth-store.offline.test.ts` test (offline mint branch), both using a store id distinct from any roster/bundle id so a wrong binding cannot pass by coincidence.
- [x] 8.1.3 GAP 3 (step 5's unconditional password-wrap write) — confirmed against device-dek-wrap spec's "Password wraps stay synchronized with the device DEK" requirement and its "Out-of-band password change recovers via the device DEK" scenario, corroborated by design §5's own prose ("later logins mutate ... users ... fields") and §7's "one extra PBKDF2 ... per login" note — CORRECT, not a deviation. Pinned with new test `dek-provisioning.test.ts` 5.13, isolated from the F9/roster-reconciliation path entirely (no roster imported) to prove the rewrite is a general policy, not scoped to stale-wrap recovery. RED-confirmed via temporary mutation (literal `if (!table.users[login])` guard), which also incidentally broke the pre-existing WU5 test 5.7 (F9) — corroborating evidence that 5.7 already implicitly required this policy.
Verify: `npx turbo run test --force --filter=@store-mgmt/web-store-pos` — 185 test files, 2433 tests, 0 failures (+4 tests over WU8's 2429, 0 new files, 0 production diff).
