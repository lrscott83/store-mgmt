...
- [x] 4.1 **Unit: null user → 403** — Mock `GetByIdAsync` → null, assert `ApiException` with `Forbidden` thrown
- [x] 4.2 **Unit: access denied → 403** — Mock `GetActiveStoresByUserIdAsync` → list without `request.StoreId`, assert `ApiException` with `Forbidden`
- [x] 4.3 **Unit: SuperAdmin bypass** — Mock `IsSuperAdmin` → true, assert `GetActiveStoresByUserIdAsync` NOT called
- [x] 4.4 **Unit: validator uses ExistsAsync** — Assert `_storeRepository.ExistsAsync` called once; `_storeByIdService` never called
- [x] 4.5 **Unit: no .NotNull() on Guid** — Assert rule chain has `.NotEmpty()` but not `.NotNull()`
- [x] 4.6 **Integration: happy path** — Valid SuperAdmin sets store ID → 200, `SelectedStoreId` updated in DB
