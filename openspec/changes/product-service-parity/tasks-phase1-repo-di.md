...
- [ ] 1.1 RED/GREEN: `getProductCategoryById(id)` (repo.ts:51-53) → match or `undefined`.
- [ ] 1.2 RED/GREEN: `getProductCategoryByName(name)` (55-57).
- [ ] 1.3 RED/GREEN: `getProductCategories()` — ALL, sorted ascending by `order` (59-61).
- [ ] 1.4 RED/GREEN: `getAvailableProductCategories()` — `isActive`-only, sorted (63-65).
- [ ] 1.5 RED/GREEN: `hasAnyCategory()` / `hasAnyAvailableCategory()` (67-69, 25-27).
- [ ] 1.6 RED/GREEN: `addProductCategory(name, order, isActive)` — name-collision fails, no
...
- [ ] 1.7 RED/GREEN: `addProductCategoryByName(name)` — generated id, next order via private
      `getNextOrder()` (94-103), always `isActive: true`, returns new id or `null`.
- [ ] 1.8 RED/GREEN: `updateProductCategory(id, name, order, isActive)` — not-found fails
      (122-124); name-collision excluding self fails (126-128); success updates + same
      order-shift-then-reassign (130-136).
- [ ] 1.9 RED/GREEN: `activateProductCategory(id)` / `deactivateProductCategory(id)` — **1-param**
      (flagged #1, dead-param fix), toggle ONLY `isActive` via private
      `updateProductCategoryActive` (139-148).
- [ ] 1.10 RED/GREEN: `getCategoriesJson()` (172-174); `addImportedProductCategory`/
      `updateImportedProductCategory`/`updateCategories`/`setInitCategories`/
      `getStorageCategoriesMap` (29-44, 105-119) — sync/import parity.
- [ ] 1.11 Confirm NO `upsert`/`remove` exist on the class (Exact-Surface Rule).
- [ ] 1.12 Gate: `pnpm test`, `tsc --noEmit`; commit
...
- [ ] 2.1 RED: `packages/domain/src/errors/__tests__/product-category-errors.test.ts` asserts
      `NameExists`/`NotExists` codes+descriptions verbatim (product-category.errors.ts:7-14).
- [ ] 2.2 GREEN: create `packages/domain/src/errors/product-category-errors.ts` (`as const
      satisfies Record<string, BaseError>`, same pattern as `product-errors.ts`).
- [ ] 2.3 GREEN: export from `packages/domain/src/index.ts`.
- [ ] 2.4 Gate; commit `feat(domain): port ProductCategoryErrors (byte-identical Angular parity)`.
...
- [ ] 3.1 RED/GREEN: `getAvailableProducts()` (product.repository.ts:46-48) — `isActive`-only,
      unsorted, needed by WU4.3's `inventory-today-sale-service.ts` re-point.
- [ ] 3.2 RED/GREEN: `getProductByName(name)` (59-61); `getProductByBarcode(barcode)` (63-66,
      empty-barcode → `undefined`); `hasAnyProduct()` (68-70).
- [ ] 3.3 RED/GREEN: `getProductsByCategoryId(categoryId)` sorted by `order` (72-76);
      `getAvailableToSaleProductsByCategoryId(categoryId)` — `isActive && availableToSale`,
      sorted (78-82).
- [ ] 3.4 RED/GREEN: `hasAnyAvailableToSaleProduct()` —
      `categoryRepository.hasAnyAvailableCategory() && some(isActive && availableToSale)` (84-86).
- [ ] 3.5 RED/GREEN: `deleteProduct(id)` — soft-delete: `isActive=false` + `updatedDate`/
      `updatedByName` stamp, returns `true`; missing id → `false`, no throw (88-98).
- [ ] 3.6 RED/GREEN: `addProductData(id, categoryId, name, price, businessId, order, isActive,
...
- [ ] 3.7 RED/GREEN: `addProduct(...9 args)` — delegates to `addProductData` with generated id
      (148-171); `addImportedProduct(product)` (173-185).
- [ ] 3.8 RED/GREEN: `updateProduct(id, categoryId, name, price, businessId, order, isActive,
...
- [ ] 3.9 RED/GREEN: `setDiscountFromInvantory(id, discountFromInvantory)` — only that flag, no
      audit stamps; missing id fails `ProductErrors.NotExists` (261-268).
- [ ] 3.10 RED/GREEN: `activateProduct(id)` / `deactivateProduct(id)` — toggle ONLY `isActive`, no
      audit stamps, via private `updateProductActive` (270-285).
- [ ] 3.11 RED/GREEN: `updateProducts(map)` / `setInitProducts(map)` / `getProductsJson()`
      (26-34, 301-303) — sync/import parity.
- [ ] 3.12 Confirm the class still has NO `upsert`/`remove`.
- [ ] 3.13 Gate; commit `feat(web-store-pos): extend ProductRepository with validations, order-shift, soft-delete, activate/deactivate`.
...
- [ ] 4.1 `today-quantities.tsx:64` `categorySvc.getById(categoryId)?.order ?? 999` →
      ...
      needs them.
- [ ] 4.2 `today-sales-profit.tsx:87` / `:91-93` — same two swaps (mirrors
      inventory-today-sales-profit.component.ts, same pattern as WU4.1).
- [ ] 4.3 `entries.tsx:93-95` and `today-entries.tsx:33-35` — `productSvc.getAll()` →
...
- [ ] 4.4 `inventory-today-sale-service.ts:58,63,69` — replace the injected
...
- [ ] 4.5 Update `inventory-routes.test.tsx` / `inventory-today-sale-service.test.ts` mocks:
...
- [ ] 5.1 Confirm `ProductCategoryRepository` and `ProductRepository` both declare NO
      `upsert`/`remove`.
- [ ] 5.2 Confirm `ProductRepository`'s constructor now depends on `ProductCategoryRepository`
      (matches product.repository.ts:21-24).
- [ ] 5.3 Grep-confirm the four WU4 files no longer import `ProductOfflineService`/
...
- [ ] 5.4 Full gate — domain: `pnpm test`, `tsc --noEmit`, build. web-store-pos: `pnpm test`,
      `tsc --noEmit`, `pnpm build` — all green.
- [ ] 5.5 Update this file with commit hashes.
