...
- [x] 2.1 RED: new `app/reports/lib/pdf/inventory-today-sale-pdf.test.ts` (flat, matching directory convention) — module import alone does not trigger `jspdf`/`jspdf-autotable` module resolution (assert no eager top-level import; smoke-test the exported function signature) (Requirement: Lazy Code-Split Export)
- [x] 2.2 RED: same file, `jspdf`+`jspdf-autotable` mocked — calling `exportInventoryTodaySalePdf(rows)` performs `await import('jspdf')` and `await import('jspdf-autotable')` INSIDE the function body (not at module top-level). Dropped the design's unused `meta` param — Angular's admin header has zero dynamic data (all 4 lines are static blank placeholders), so no metadata object is ever consumed.
- [x] 2.3 RED: same file — generated document includes the verbatim 4-line administrative header (Empresa/Procedencia; Unidad/UBA/OEE/date; Departamento/Balance/BAT; Firma del Administrador) positioned above the title, followed by title `INVENTARIO A PRECIO DE VENTA` and a 13-column `autoTable` call (assert mocked `autoTable` invoked with 13 header columns matching spec column order) (Requirement: Verbatim Administrative Header)
- [x] 2.4 RED: same file — mocked `doc.output`/`URL.createObjectURL`/`window.open` are all invoked (real PDF produced and opened, not a stub) (Requirement: Working Export Action)
- [x] 2.5 GREEN: create `app/reports/lib/pdf/inventory-today-sale-pdf.ts` — `exportInventoryTodaySalePdf(rows: InventoryTodaySaleRow[]): Promise<void>`; lazy `await import('jspdf')` + `await import('jspdf-autotable')` inside the function; landscape letter; encabezado @ (40, 30+i*14) 10pt bold ×4 lines; title @ (300,100); `autoTable` 13 cols, fontSize 8, fillColor [220,220,220], margin.top 120, `didDrawPage` header/title redraw for page 2+; blob → `createObjectURL` → `window.open`. Added `jspdf@^4.2.1` + `jspdf-autotable@^5.0.8` to `apps/web-store-pos/package.json` (jspdf-autotable v5's API is `autoTable(doc, options)` default export, not the old `doc.autoTable()` prototype-patch Angular's commented code used — functionally equivalent, called the same way per-call).
- [x] 2.6 REFACTOR: run `inventory-today-sale-pdf.test.ts` green; `pnpm test`; `pnpm -C apps/web-store-pos exec tsc --noEmit`; `pnpm -C apps/web-store-pos build` clean. Bundle-analysis of the jspdf chunk is DEFERRED to Batch 2 (Unit C) — no route wires `inventory-today-sale-pdf.ts` in yet in this batch, so Vite doesn't include it in any output chunk at all (confirmed: zero "jspdf" references in `build/client/assets/*.js`); the code-split contract (dynamic `import()` only inside the async function body, never top-level) is verified at the source level here. Commit `feat(web-store-pos): add lazy-loaded inventory-today-sale PDF export`
