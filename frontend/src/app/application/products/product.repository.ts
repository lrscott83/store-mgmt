...
export class ProductRepository {
    ...
    constructor(private categoryRepository: ProductCategoryRepository, private authService: AuthService) {
    }
    ...
    addProduct(categoryId: string, name: string, price: number, businessId: string, order: number,
        isActive: boolean, availableToSale: boolean, discountFromInvantory: boolean): Result {
        return this.addProductData(Guid.create().toString(), categoryId, name, price, businessId, order,
            isActive, availableToSale, discountFromInvantory);
    }

    addImportedProduct(product: Product): Result {
        return this.addProductData(product.id, product.categoryId, product.name, product.price, product.businessId, product.order, product.isActive, product.availableToSale, product.discountFromInvantory);
    }
