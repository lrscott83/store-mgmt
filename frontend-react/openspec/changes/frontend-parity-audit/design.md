...
Confirm factory pattern: React `createService(offline, online)` returns offline when `USE_ONLINE_SERVICE=false`. Enumerate Angular `application/*` services → map each method to React `*-offline-service.ts`; matrix method-level gaps. PWA cross-cutting = DESIGN QUESTIONS (Angular has `_services/connection`, `_services/download-manager`, `_services/update`, `_services/usage-tracker`; React coverage unconfirmed — grep found only scattered refs). Resolve these in the Sync foundation stage before declaring any module at parity. NOTE (resolved, Stage 1 cart batch): `_services/order/shopping-cart.service.ts`'s cart UI/checkout FLOW parity is Stage 1 (Sales) scope, not Sync. Its cross-cutting inventory-availability-on-increase/decrease check was subsequently MOVED to Stage 2 (Inventory) — it depends on `InventoryOfflineService`/stock data Stage 2 owns, so it is an Inventory dependency, not a Sync one (see tasks.md 2.5). Login/auth parity (no dedicated module stage) was also folded into Stage 2 (tasks.md 2.6): the post-login `navigateToUserHome` redirect and the `sale.tsx` availability gate both depend on product-availability, so they co-locate with Inventory; the pure login-form view parity rides along there for scheduling only. Stage 6 Sync keeps ONLY non-inventory PWA services (connection, download-manager, SW update, usage-tracker).
