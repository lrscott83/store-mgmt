...
describe('channel-rate-offline-service — at-rest encryption seam (entity-at-rest-encryption)', () => {
  beforeEach(() => {
    localStorage.clear();
    clearDek();
    clearRoster();
  ...
  it('a provisioned-but-locked read never destroys existing ciphertext', () => {
    ...
    const lockedService = new ChannelRateOfflineService(storeId);
    expect(() => lockedService.getStorageChannelRates()).toThrow(MissingDataKeyError);
