...
export const userHttpService = {
  async getUsers(): Promise<BaseResponseModel<User[]>> {
    const response = await apiClient.get<BaseResponseModel<User[]>>('/v1/users/all/true');
  ...
  async createUser(payload: CreateUserPayload): Promise<BaseResponseModel<boolean>> {
    const response = await apiClient.post<BaseResponseModel<boolean>>('/v1/storeusers', payload);
    return response.data;
  },
  ...
  async editUser(
    ...
    const response = await apiClient.put<BaseResponseModel<boolean>>(`/v1/users/${id}`, payload);
    return response.data;
  },

  async activateUser(id: string, isActive: boolean): Promise<BaseResponseModel<boolean>> {
    const response = await apiClient.post<BaseResponseModel<boolean>>('/v1/users/activate', {
      id,
      isActive,
    });
    return response.data;
  ...
  async deleteUser(id: string): Promise<BaseResponseModel<boolean>> {
    const response = await apiClient.delete<BaseResponseModel<boolean>>(`/v1/users/${id}`);
    return response.data;
  },
};
