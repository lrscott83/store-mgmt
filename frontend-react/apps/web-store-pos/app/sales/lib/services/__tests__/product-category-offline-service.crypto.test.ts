...
describe('product-category-offline-service — at-rest encryption', () => {
  beforeEach(() => {
    localStorage.clear();
    clearDek();
    clearRoster();
  ...
  it('plaintext mode: unprovisioned device writes/reads raw JSON', async () => {
    const service = new ProductCategoryOfflineService(storeId);
  ...
  it('provisioned + unlocked write produces ciphertext, round-trips', async () => {
    ...
    const service = new ProductCategoryOfflineService(storeId);
    await service.createProductCategory('Encrypted', 1, true);
  ...
  it('locked read throws MissingDataKeyError without destroying data', async () => {
    ...
    const service = new ProductCategoryOfflineService(storeId);
    // Create while unlocked — auto-init writes the key
    await service.createProductCategory('Test', 1, true);
    ...
    clearDek();
    const lockedService = new ProductCategoryOfflineService(storeId);
    // Error is thrown synchronously before Promise.resolve() wraps it
    expect(() => lockedService.getProductCategories()).toThrow(MissingDataKeyError);
