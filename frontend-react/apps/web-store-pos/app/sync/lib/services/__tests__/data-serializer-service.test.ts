...
describe('DataSerializerService', () => {
  ...
  describe('T2 — v2 envelope: meta.json + all data entries', () => {
    it('produces meta.json plus exactly the 12 data entries', async () => {
      const svc = makeService();
      const payload = await svc.export(PASSWORD);
    ...
    it("EDataFileName mirrors Angular's PascalCase member names with unchanged string values, plus the daily-exchange-rate seventh entry, the three warehouses entries and the two elaboration entries", () => {
