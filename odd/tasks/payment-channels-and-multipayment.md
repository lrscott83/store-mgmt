...
- **AUTORIZACIÓN PENDIENTE — 1 spec E2E quedará rojo**: T19a cambió a propósito los textos del selector de la página de canales ("Efectivo" → "Efectivo (CUP)", "Transferencia" → "Transferencia (CUP)", "Zelle" → "Zelle (USD)"). El spec **creado en este mismo módulo**, `e2e/channel-rates-catalogue.spec.ts`, asserta los textos viejos en las líneas **113** (`['Efectivo','Transferencia']`), **117-121** (`['Efectivo','Zelle','Transferencia']`) y **125** (`['Transferencia']`) ⇒ **va a fallar** cuando se corra con el backend arriba. Arreglarlo son **3 líneas**, pero es un E2E existente → **requiere tu autorización explícita** (la que diste en D8 cubre solo `multipayments.spec.ts`). Los otros tres specs que entran a esa página eligen por `option value` y **siguen pasando**.
