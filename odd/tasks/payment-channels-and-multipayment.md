...
- [x] **T23** — **E2E: impacto y autorización.** Specs existentes que T20/T21/T22 dejan desactualizados. **Ruta:** delegada (writer) con **autorización explícita del usuario** (2026-09-24); **sin ejecutar ningún E2E**. Commits `5dedb09b` (parte 1: campo legacy "Pago" + radios → **14 archivos**) y `ad8b23c4` (parte 2: select de canal por fila + sección de Configuraciones + formulario de tasas en popup → **7 archivos**).
  **AC:** ✅ los **21 archivos** actualizados (locadores y pasos, **sin debilitar ninguna aserción**); `playwright test --list` → **339 tests en 100 archivos, 0 ejecutados** (compila todo); `pnpm typecheck` 5/5 · `pnpm lint` 4/4; `git status --porcelain -- frontend-react/e2e/` → solo los autorizados. **Excepción reportada honestamente:** en `multipayments-cart-v2.spec.ts` el paso "cambiar la moneda re-fija el método" deja de existir como acción separada (ahora hay un solo select de canal); la cobertura de selección/conversión/recálculo se conserva.
- [x] **T24** — **Revertir el alcance sin módulo en el carrito** (corrección del usuario 2026-09-24): sin MultiPayments el carrito debe verse **como antes** — vuelve el **bloque legacy** (input "Pago" + radios de forma de pago). La lista nueva, el select único de canal, la fila por defecto, el popup y la papelera quedan **solo con el módulo 16**. Los radios muestran: con **MultiMonedas** activo, **los canales habilitados de la moneda de la venta**; sin MultiMonedas, **solo Efectivo y Transferencia (CUP)**. El **layout se queda para todas** las tiendas. **Ruta:** delegada (writer). Commits `ce4ac792` (código) y `6809366a` (E2E).
  **AC:** ✅ `MultiPaymentList` vuelve a `return null` sin el módulo 16 y el carrito vuelve a renderizar el bloque legacy (input "Pago" + "Vuelto" + radios `payment-method-*`); con el módulo, el comportamiento de T15/T21 queda **byte a byte igual**; los radios usan el compositor existente (catálogo por moneda → gate de MultiMonedas → config por canal de T20) ⇒ con MultiMonedas y venta CUP: Efectivo + Transferencia (CUP), y sin MultiMonedas los mismos dos; `salePaymentMethodLabel` **intacto**; el layout (T15) se conserva; los 14 E2E de T23 parte 1 se revirtieron con `git revert 5dedb09b` y **ningún otro archivo de `e2e/` cambió**.
- [x] **T25** — Limpieza: quitar las **semillas de datos inertes** que T21 dejó en los tests unitarios (`payments: [paymentRowOf(...)]` / `setPayments: vi.fn()` en tests cuya tienda **no** tiene el módulo 16). **Ruta:** delegada (writer). Commit `9d25d365`.
  **AC:** ✅ se quitaron **9 semillas inertes** (todas en `cart-shell.test.tsx`, tiendas sin el módulo 16) más el helper `paymentRowOf`, que quedó sin uso; verificado empíricamente: **85/85 tests antes y después**, con los mismos nombres y las mismas aserciones. **Ninguna semilla resultó cargante** (las del módulo 16 se conservaron intactas). **Veredicto sobre `a89a38a2`: ya es removible** — su semilla existía solo por el guard de T21, que T24 revirtió; el test pasa por la razón correcta (el guard legacy omite la validación del pago si `payment` es `undefined`).
...
- **T23 — CERRADO (autorizado y hecho, 2026-09-24)**: los **21 archivos** quedaron al día con commits `5dedb09b` (los 13 specs del campo legacy "Pago" + `payment-methods.spec.ts` + `mayorista-sale.spec.ts` + `support/elaboration-flow.ts`) y `ad8b23c4` (los 7 restantes: selects por fila, sección de Configuraciones y formulario de tasas en popup). **Ningún E2E ejecutado** — verificación por `playwright test --list` (339 tests / 100 archivos, 0 ejecutados), `pnpm typecheck` y `pnpm lint`. Ninguna aserción de comportamiento fue debilitada.
- **Flake del entorno**: la corrida completa de Vitest durante T22 reportó 22 fallos en 10 archivos **ajenos a lo tocado** (`auth-store.offline`, `decryption-failure-policy` TDZ). Verificado en aislamiento por el writer (10/10) y por el orquestador (1 file passed) ⇒ **contención del entorno, no regresión**. (En T23/T24 volvió a aparecer el mismo patrón: otro checkout del usuario — `qa-env` — corriendo su propio loop de Vitest satura la CPU; los fallos siempre son tests distintos y ajenos, que pasan en aislamiento.)
- **T24 — comportamiento preexistente que se conserva**: con el módulo 16 y el carrito **vacío**, el bloque legacy sigue renderizándose (el ternario depende de `items.length > 0`). Es el comportamiento previo a T21, no una decisión nueva — se documenta para que no sorprenda.
- **T25 — semillas inertes: CERRADO**: las **9** semillas que T21 dejó en tests sin el módulo 16 se eliminaron (commit `9d25d365`), con **85/85 antes y 85/85 después**. Ya no queda esa deuda de limpieza.
- **E2E sin ejecutar**: desde T13 no se corre ningún E2E. Los **346 tests en 103 archivos** compilan y coleccionan (`playwright test --list`), pero **no se ejecutan** (backend detenido a pedido tuyo). Toda la verificación E2E de T10/T11/T23/T24 es por colección + lectura.
...
- 2026-09-24 (cont. 7): el usuario autorizó actualizar los specs E2E desactualizados, **sin ejecutar E2E**. **T23 cerrado** en dos partes: `5dedb09b` (14 archivos: el campo legacy "Pago" y los radios) y `ad8b23c4` (7 archivos: select de canal por fila, sección de Configuraciones y formulario de tasas en popup). Verificación por colección (`playwright test --list` → 339 tests / 100 archivos, 0 ejecutados), typecheck 5/5 y lint 4/4. Sin debilitar aserciones. **Módulo completo: T1–T23.**
- 2026-09-24 (cont. 8): merge de `origin/test` en `dev` (50 commits, limpio de texto) y push. Aparecieron 3 fallos post-merge, los tres **tests desactualizados** (no regresiones): `36177ce9` y `a89a38a2`. Verificado que el **carrito queda con nuestra lógica** (T21/Parte A + T15) y que la regla de negocio de `test` (mayorista por `orderType`) sigue intacta.
- 2026-09-24 (cont. 9): el usuario corrigió el alcance — **todo el trabajo del carrito era solo para el caso CON el módulo 16**. **T24 implementado** (`ce4ac792`, `6809366a`): la lista vuelve a estar gated por el módulo y sin él vuelve el bloque legacy (input "Pago" + radios); los radios muestran los canales habilitados **de la moneda de la venta** (con MultiMonedas) o solo Efectivo + Transferencia (CUP) (sin MultiMonedas); el layout se conserva para **todas** las tiendas. E2E de T23 parte 1 revertido con `git revert`. Workspace verde: 12/12 tareas, 302 archivos / 4562 tests.
- 2026-09-24 (cont. 10): **T25 — limpieza de semillas inertes** (commit `9d25d365`): 9 semillas que T21 había dejado en tests sin el módulo 16 + el helper `paymentRowOf` sin uso; **85/85 antes y después**, sin tocar ninguna aserción. La semilla de `a89a38a2` (arreglo del merge) quedó confirmada como removible. **Ya no queda deuda de limpieza de este tipo.**
...
- T23 — `git show --stat 5dedb09b` → **14 archivos** (13 specs + `support/elaboration-flow.ts`); `git show --stat ad8b23c4` → **7 archivos**. Mapeo: grupo 1 `spinbutton 'Pago'` → `multi-payment-amount` con el mismo valor; grupo 2 radios `payment-method-2` → `multi-payment-channel` `'0|2'`; grupo 3 `multi-payment-method`+`-currency` → `multi-payment-channel` (valor `currencyKey` `'0|0'`, `'1|0'`); grupo 4 "Formas de pago"→"Métodos de pago" y nombres con moneda; grupo 5 `channel-rate-add` antes del formulario.
- T23 — **spot check del orquestador**: `pnpm exec playwright test --list` (solo colección, 0 ejecutados) → sin errores; con typecheck 5/5 y lint 4/4 ya reportados por los writers. `git status --porcelain -- frontend-react/e2e/` → **vacío** tras los commits.
- T24 — `pnpm --filter @store-mgmt/web-store-pos exec vitest run app/shared/components/multipayments/__tests__/multi-payment-list.test.tsx app/shared/components/__tests__/cart-shell.test.tsx` → 2 files, **112/112 tests**. `pnpm turbo run typecheck lint test` → **12/12 tareas**, `@store-mgmt/web-store-pos` **302 archivos / 4562 tests** pasados. `pnpm typecheck` 5/5 · `pnpm lint` 4/4. `pnpm exec playwright test --list` → **346 tests en 103 archivos, 0 ejecutados**. `git status --porcelain -- frontend-react/e2e/` → **vacío** tras el commit.
- T24 — revert E2E: `git revert --no-commit 5dedb09b` auto-mergeó limpio sobre los cambios posteriores (`a638534d`/`c2ee8746`/`6a077455`), así que las aserciones de formato de moneda se conservaron. Los 14 archivos son exactamente el set autorizado; `multipayments*.spec.ts`, `channel-rates-catalogue.spec.ts` y `payment-methods-config*.spec.ts` **no se tocaron**.
- T25 — `pnpm --filter @store-mgmt/web-store-pos exec vitest run app/shared/components/__tests__/cart-shell.test.tsx`: **85/85 antes y 85/85 después** (mismos tests, mismas aserciones). `pnpm turbo run typecheck lint` → **9/9** · `pnpm turbo run test` → 5/5, `@store-mgmt/web-store-pos` **302 archivos / 4562 tests**. `git status --porcelain -- frontend-react/e2e/` → **vacío** (solo se tocó un archivo de test, −32 líneas).
